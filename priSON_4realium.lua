...
local skyboxes = {
    ["Scary Sky"] = {
        SkyboxUp = "rbxassetid://1102142750",
        SkyboxRt = "rbxassetid://1102142750",
        SkyboxLf = "rbxassetid://1102142750",
        SkyboxFt = "rbxassetid://1102142750",
        SkyboxBk = "rbxassetid://1102142750",
        SkyboxDn = "rbxassetid://1102142750",
    },
    ["Classic Sky"] = {
...
if services.UserInputService.TouchEnabled then
    Window = Library:CreateWindow({
        Title = "4realium",
        Footer = "By Peliy11 | discord.gg/fcQJNYMyFP | v1.0",
...
local DiscordInvite = "PASTE_DISCORD_INVITE_HERE"

local tabs = {
    HomeTab = Window:AddTab("Home", "home"),
...
local groups = {
    UpdatesBox = tabs.HomeTab:AddLeftGroupbox("Updates"),
    PlayerInfoBox = tabs.HomeTab:AddRightGroupbox("Player"),

    Misc = tabs.CombatTab:AddLeftGroupbox("Miscellaneous"),
    ...
    ItemGiverBox = tabs.WeaponTab:AddLeftGroupbox("Item Giver"),
    ...
    EffectCustizationBox = tabs.SkinsTab:AddRightGroupbox("Effect Customization"),
    ...
    Vehicles = tabs.WorldTab:AddRightGroupbox("Vehicles"),
}

groups.UpdatesBox:AddLabel("v1.3", true)
groups.UpdatesBox:AddLabel("- Added Home tab", true)
groups.UpdatesBox:AddLabel("- Moved Effect Customization to Mods", true)
groups.UpdatesBox:AddLabel("- Removed Gun Mods from Weapons", true)

groups.PlayerInfoBox:AddLabel("Name: " .. LocalPlayer.Name, true)
groups.PlayerInfoBox:AddLabel("Display Name: " .. LocalPlayer.DisplayName, true)
groups.PlayerInfoBox:AddButton({
    Text = "Join Discord",
    Func = function()
        if DiscordInvite == "https://discord.gg/fcQJNYMyFP" then
            Library:Notify("Discord invite is not configured", 4)
            return
        end
        ...
        if type(setclipboard) == "function" then
            ...
            Library:Notify(DiscordInvite, 5)
        end
    end
})
...
groups.ItemGiverBox:AddButton({
    Text = "Give Key Card",
    Func = function()
        local keycard = services.ReplicatedStorage.Tools:FindFirstChild("Key card")
        if not keycard then
            Library:Notify("Key card not found", 3)
            return
        end
        if LocalPlayer.Backpack:FindFirstChild("Key card") or (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Key card")) then
            Library:Notify("Already have Key card", 3)
            return
        end
        local clone = keycard:Clone()
        clone.Parent = LocalPlayer.Backpack
        Library:Notify("Key card given", 3)
    end
})
...
local HitboxIgnoreGuards = { Value = false }
local HitboxIgnoreCriminals = { Value = false }
local HitboxIgnoreInnocent = { Value = false }
local HitboxIgnoreHostile = { Value = false }
local HitboxIgnoreTrespass = { Value = false }
local HitboxIgnoreForceField = { Value = false }
...
local SilentIgnoreGuards = { Value = false }
local SilentIgnoreCriminals = { Value = false }
local SilentIgnoreInnocent = { Value = false }
local SilentIgnoreHostile = { Value = false }
local SilentIgnoreTrespass = { Value = false }
local SilentIgnoreForceField = { Value = false }
...
local function isSilentAimHostile(character)
    local success, value = pcall(function()
        return character:GetAttribute("Hostile")
    end)
    if success and value ~= nil then
        return value == true
    end
    local hostileValue = character:FindFirstChild("Hostile")
    return hostileValue and hostileValue:IsA("BoolValue") and hostileValue.Value or false
end

local function canSilentAimDamage(player)
    local character = player.Character
    local humanoid = character and character:FindFirstChildOfClass("Humanoid")
    if not humanoid or humanoid.Health <= 0 or character:FindFirstChild("ForceField") then
        return false
    end
    if SilentTeamCheck.Value then
        if player.Team and player.Team == LocalPlayer.Team then
            return false
        end
        if LocalPlayer.Team and LocalPlayer.Team.Name == "Guards" and not isSilentAimHostile(character) then
            return false
        end
    end
    if player.Team then
        ...
        if SilentIgnoreInnocent.Value and player.Team.Name == "Inmates" and not isSilentAimHostile(character) then
            return false
        end
    end
    if SilentIgnoreHostile.Value and isSilentAimHostile(character) then
        return false
    end
    if SilentIgnoreTrespass.Value and character:GetAttribute("Trespassing") == true then
        return false
    end
    if SilentIgnoreForceField.Value and character:FindFirstChild("ForceField") then
        return false
...
local function getSilentAimTarget(origin)
    ...
    local bestDistance = slientCircle.Radius
    local bestPart
    for _, player in ipairs(services.Players:GetPlayers()) do
        if player ~= LocalPlayer and canSilentAimDamage(player) then
            local character = player.Character
            local part = character and character:FindFirstChild(BodyPartsDropdown.Value)
            if part then
                if (origin - part.Position).Magnitude <= 300 then
                    local screenPosition, onScreen = cameraObject:WorldToViewportPoint(part.Position)
                    if onScreen and (part.Position - cameraObject.CFrame.Position):Dot(cameraObject.CFrame.LookVector) > 0 then
                        local screenDistance = (Vector2.new(screenPosition.X, screenPosition.Y) - services.UserInputService:GetMouseLocation()).Magnitude
                        if screenDistance < bestDistance then
                            if not SilentWallCheck.Value or isVisible(part, origin) then
                                bestPart = part
...
local function getCastOrigin(first, second, third)
    if typeof(first) == "Vector3" then return first end
    if typeof(second) == "Vector3" then return second end
    if typeof(third) == "Vector3" then return third end
end

local function normalCast(first, second, third)
    local origin = getCastOrigin(first, second, third)
    local aim
    local tool
    if typeof(first) == "Vector3" then
        aim = typeof(second) == "Vector3" and second or (typeof(third) == "Vector3" and third)
        tool = typeof(second) == "Instance" and second or (typeof(third) == "Instance" and third)
    else
        tool = first
        aim = typeof(third) == "Vector3" and third or (typeof(second) == "Vector3" and second)
    end
    ...
    if typeof(tool) == "Instance" then
        local success, configuredRange = pcall(function()
            return tool:GetAttribute("Range")
        end)
        if success and type(configuredRange) == "number" then
...
local function hookCast(first, second, third)
    if not SilentAimToggle.Value or not didHit() then
        return normalCast(first, second, third)
    end
    local origin = getCastOrigin(first, second, third)
    if origin then
        local target = getSilentAimTarget(origin)
...
if type(filtergc) == "function" and type(hookfunction) == "function" then
    local castRayFunctions = filtergc("function", { Name = "castRay" }, true)
    if type(castRayFunctions) == "function" then
        castRayFunctions = { castRayFunctions }
    end
    if type(castRayFunctions) == "table" then
        for _, castRayFunction in pairs(castRayFunctions) do
            if type(castRayFunction) == "function" then
                pcall(hookfunction, castRayFunction, hookCast)
            end
        end
    end
end
...
local VehicleFLY = groups.Vehicles:AddToggle("VehicleFLY", {
    Text = "Vehicle Fly",
    Default = false,
    Callback = function(Value)
...
local VehicleFLYSlider = groups.Vehicles:AddSlider("VehicleFLYSlider", {
    Text = "Height Slider",
    Default = 0,
    Min = 0,
    Max = 100,
...
groups.Vehicles:AddDivider()

groups.Vehicles:AddButton({
    Text = "Flip Car",
    Func = function()
...
local VehicleSPEED = groups.Vehicles:AddToggle("VehicleSPEED", {
...
local VehicleMaxSpeed = groups.Vehicles:AddSlider("VehicleMaxSpeed", {
...
local VehicleTorque = groups.Vehicles:AddSlider("VehicleTorque", {
    Text = "Vehicle Torque",
    Default = 5,
    Min = 1,
    Max = 50,
...
local VehicleTurnSpeed = groups.Vehicles:AddSlider("VehicleTurnSpeed", {
...
refreshlist()
local carlistdropdown = groups.Vehicles:AddDropdown("carlistdropdown1", {
    Text = "Car List",
    Default = "",
    Values = listcars
})
...
SaveManager:SetFolder("4realium")
